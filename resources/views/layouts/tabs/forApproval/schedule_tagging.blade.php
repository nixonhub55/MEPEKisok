            ...
            <div class="modal-dialog modal-xl">
                  <div class="modal-content">
                        <div class="modal-header bg-primary text-white">
                              <h5 class="modal-title" id="teModalLabel">Schedule Tagging Application Details</h5>
                              ...
                              <tbody>
                                    ...
                                    <tr>
                                          <td>
                                                ...
                                                @else
                                                <center>
                                                <input class="form-check-input" id="chkbox{{$num}}" type="checkbox" value="{{$rows->enc_id}}"  data-database="{{$rows->r_srcDb}}"  name="{{$rows->stAppNo}}" onclick="return CheckMe(1,this.id,'selectAll','datatablesPending')">
                                                </center>
                                                @endif
                                                
                                          </td>
                                          <td>{{$rows->stAppNo}}</td>
                                          <td>{{$rows->stAppDate}}</td>
                                          <td>{{$rows->stID}}</td>
                                          <td>{{$rows->stName}}</td>
                                          <td>{{$rows->stCostCenter}}</td>
                                          <td>{{$rows->department}}</td>
                                          <td>{{$rows->stAppDate}}</td>  
                                          <td>{{$rows->stReason}}</td> 

                                          <td id="td{{$rows->stAppNo}}"> 
                                                <label class="fixed_lbl" id="lbl{{$rows->stAppNo}}" for="txt{{$rows->stAppNo}}"></label>
                                                <input type="text" id="txt{{$rows->stAppNo}}" class="form-control" maxlength="200"> 
                                          </td> 

                                          <td>
                                                <div class="btn-container"> 
                                                      <button id="{{$rows->stAppNo}}" class="btn btn-action btn-view" title="More Details" onclick="view_ta_details('{{$rows->enc_id}}',this.id,'{{$rows->enc_pay}}','{{$rows->r_srcDb}}')"><i
                  ...
                  <div class="table-container">   
                        <div class="container"> 
                              <div class="row bg-light rounded p-2"> 
                                    ...
                                    <button class="btn btn-primary" id="btnFilter" onclick="Filter_History(1,this.id,8)">
...
<script>
      ...
      var selectedID,this_mode,selectedDB;
      var chkbox_list = [];
      var chkbox_list_org = [];
      ...
      function view_ta_details(id,objID,enc_pay,r_srcDb){ 
      ...
      async function AppoverResponse(pint_mode,val,objID) {
            ...
            if (pint_mode==0){
                  ...
                  //GlovalHTMLObjLoading(0,objID);  
                  //console.log(response);
                  window.location.href='{{url("/scheduleTagging_approval")}}';
            }
            GlovalHTMLObjLoading(0,objID);  
            FadeOut();
      }
      ...
      async  function ConfirmResponseAll(r_code,objID){ 
            ...
            for (let i = 0; i <= num; i++) { 
                  if(document.getElementById(chkbox_list[i])){
                        var checkbox = document.getElementById(chkbox_list[i])
                        var database = checkbox.getAttribute('data-database');
                        console.log(database);
                        var id = checkbox.value; 
                        var remarks = document.getElementById('txt'+document.getElementById(chkbox_list[i]).name).value;
                        items.push({"id":id,"remarks":remarks,"_srcDb":database});  
                  }
            } 
