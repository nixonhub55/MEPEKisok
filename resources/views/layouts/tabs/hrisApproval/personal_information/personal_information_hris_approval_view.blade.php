<div class="modal-header border-bottom">
    <h5 class="modal-title">Personal Information Changes</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
</div>

<div class="modal-body modal-large">

@if(!empty($personalInformationNew))

    @php
        $identityNew = $personalInformationNew;
        $identityOld = $personalInformationOld;
    ...
    <div class="mb-4 fs-5">
        ...
        <div><strong>Employee Id:</strong> {{ $firstRecord->Employee_Id ?? '-' }}</div>
        <div><strong>Employee Name:</strong> {{ $firstRecord->Employee_Name ?? '-' }}</div>
    ...
    @foreach($identityNew as $idx => $newRecord)
        ...
        <div class="mb-4">
            <button class="btn btn-primary btn-approve"
                    data-request-no="{{ $requestKey }}"
                    data-employee-id="{{ $newRecord->Employee_Id ?? '' }}"
                    onclick="confirm_approve('{{ $requestKey }}', '{{ $newRecord->Employee_Id ?? '' }}')">
                    <i class="fa fa-spinner fa-spin" style="display:none;"></i>
                    Approve
            </button>

            <button class="btn btn-danger btn-disapprove"
                    data-request-no="{{ $requestKey }}"
                    data-employee-id="{{ $newRecord->Employee_Id ?? '' }}"
                    onclick="confirm_disapprove('{{ $requestKey }}', '{{ $newRecord->Employee_Id ?? '' }}')">
                    <i class="fa fa-spinner fa-spin" style="display:none;"></i>
                    Disapprove
            </button>
        </div>
...
<script>
    ...
    function submit_approval() {
        ...
        $.ajax({
            url: '{{ route('hris_approval_personalinformation_submit') }}',
            type: "POST",
            cache: false,
            data: {
                _token: '{{ csrf_token() }}',
            ...
            error: function(xhr) {
                const errorMsg = xhr.responseJSON?.message || `Error occurred during ${submitData.action}`;
