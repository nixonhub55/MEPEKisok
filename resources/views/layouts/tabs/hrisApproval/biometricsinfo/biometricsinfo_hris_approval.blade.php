        ...
        <div class="tab-pane fade show active" id="pending" role="tabpanel" aria-labelledby="pending-tab">
            ...
            <div class="table-container">
                <table class="table data-table" id="datatablesPending">
                    ...
                    <tbody>
                        <?php $count = 1; ?>
                        @foreach($biometricsinfo as $list)
...
$(document).ready(function () {
    ...
    $(document).on('click', '.open-atd', function () {
        ...
        $.ajax({
            url: "{{ route('hris_approval_biometricsinfo_view') }}",
            type: "POST",
            data: {
            ...
            error: function () {
                alert('Error loading biometrics info details');
            }
        });
    });
    ...
    function bulkSubmit(action) {
        ...
        $.ajax({
            url: "{{ route('hris_approval_biometricsinfo_bulk_submit') }}",
            type: "POST",
            data: { items: items, action: action, _token: "{{ csrf_token() }}" },
            success: function (response) {
    ...
    $('#btnBulkApprove').on('click',    function () { bulkSubmit('approve'); });
    $('#btnBulkDisapprove').on('click', function () { bulkSubmit('disapprove'); });
});
</script>
