<div class="modal-header border-bottom">
    <h5 class="modal-title">Employment History Changes</h5>
...
<div class="modal-body modal-large">
    ...
    @if(!empty($new))
        ...
        @php
            ...
            function hasActualChanges($newArr, $oldArr, $displayKeys)
            {
                foreach ($displayKeys as $colKey) {
                    $oldValue = findValue($oldArr, $colKey);
                    $newValue = findValue($newArr, $colKey);
                    
                    // Show if new data exists or old data exists (not just '-')
        ...
        @foreach($identityNew as $idx => $newRecord)
           ...
           @php
            ...
            {{-- ONLY SHOW TABLE IF THERE ARE ACTUAL CHANGES --}}
            @if(hasActualChanges($newArr, $oldArr, $displayKeys))
...
<script>
    ...
    function submit_approval() {
        ...
        $.ajax({
            url: '{{ route('hris_approval_employmenthistory_submit') }}',
            type: "POST",
            cache: false,
            data: {
                _token: '{{ csrf_token() }}',
