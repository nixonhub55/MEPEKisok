...
class Authentication extends Model
{
    ...
    public function get_database()
    {
        ...
        return  $databases = DB::table('v_kiosk_database_host as t1')
        ->leftJoin('db_profile as t2', 't1.name', '=', 't2.db_name')
        ->select('t2.*', 't1.hostName')
        ->where('t1.hostName', env('DB_OWNER'))
        ->get()
        ->toArray();
    }

    public function sp_supervisorsforms($spParams)
    {
        
        if(session()->get('allowmanpowerlist')==0 && session()->get('mySprVsr')){
            $spParams[3] = session()->get('mySprVsr');
        }

        return $this->exec_store_proc('sp_supervisorsforms', $spParams);
    }

    public function sp_manpowerlist($spParams)
    {
        return $this->exec_store_proc('sp_manpowerlist', $spParams);
    }
    ...
    public function sp_submit_scheduletaggingform($spParams)
    {
        return $this->exec_store_proc('sp_submit_scheduletaggingform', $spParams);
    }
    ...
    public function sp_branch_leader_staff($spParams)
    {
        return $this->exec_store_proc('sp_branch_leader_staff', $spParams); 
    }


    public function sp_get_company_settings_name_by_db($spParams)
    {
        return $this->pf_common_exec_store_proc('sp_get_company_settings_name_by_db',$spParams);
    }

    public function get_admin_databases(){  
        $response = Http::get('https://pfnew.smartbooks.ph/api/get-databases.php', [
            'test' =>1231,
        ]);
    ...
    $identity = DB::table('identity as t1')
        ->select(
            't1.code',
            't1.identityId',
            't1.firstName',
            ...
            't2.departmentCode',
            't2.positionCode', 
            't2.positionCode AS position',
             DB::raw('IFNULL(t3.username, "") AS isSprVsr'),
            't4.username as mySprVsr',
        )
        ->leftJoin('employeemovement as t2', 't1.code', '=', 't2.code')
        ->leftJoin('costcenter_authorization as t3', 't3.username', '=', 't1.identityId')
        ->leftJoin('costcenter_authorization as t4', 't4.costCode', '=', 't2.costCode')
        ->where('t1.identityId', $identityId)
        ->orderByDesc('t2.lineId')
        ->first();
    ...
    public function sp_application_movement($spParams){
        return $this->exec_store_proc('sp_application_movement', $spParams);
    }
    ...
    public function sp_get_payrollperiod_authorization($spParams){
        return $this->exec_store_proc('sp_get_payrollperiod_authorization', $spParams);
    }
    ...
    public function sp_requestattachments($spParams)
    {
        return $this->exec_store_proc('sp_requestattachments', $spParams);
    }
    ...
    public function sp_mp_application_info($spParams)
    {

        if(session()->get('allowmanpowerlist')==0){
            $spParams[0] = 10;
        }

        $identityId = session()->get('identityId');
        $data = $this->exec_store_proc('sp_mp_application_info', $spParams);  
        $num = 0;
